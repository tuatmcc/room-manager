...
import { ExitAllEntryUsersError, ExitAllEntryUsersUseCase } from "@/usecase/ExitAllEntryUsers";
...
describe("ExitAllEntryUsersUseCase", () => {
  ...
  const setup = () => {
    ...
    const useCase = new ExitAllEntryUsersUseCase(userRepository, roomEntryLogRepository);
  ...
  it("入室中のユーザーがいる場合は全員を退出させてユーザー一覧を返すこと", async () => {
    ...
    const entryLogs = [new RoomEntryLog(1, 1, now, null), new RoomEntryLog(2, 2, now, null)];
    const users = [new User(1, "discord-user-1"), new User(2, "discord-user-2")];
