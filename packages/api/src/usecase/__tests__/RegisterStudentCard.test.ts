...
describe("RegisterStudentCardUseCase", () => {
  ...
  const setup = () => {
    ...
    const useCase = new RegisterStudentCardUseCase(userRepository, studentCardRepository);
  ...
  it("新規ユーザーに対して学生証を登録できること", async () => {
    ...
    studentCardRepository.create.mockResolvedValue(new StudentCard(1, studentId, newUserId));
    ...
    expect(studentCardRepository.findByStudentId).toHaveBeenCalledWith(studentId);
    expect(studentCardRepository.findByUserId).toHaveBeenCalledWith(newUserId);
    expect(studentCardRepository.create).toHaveBeenCalledWith(studentId, newUserId);
  });
  ...
  it("既存ユーザーに対して新規学生証を登録できること", async () => {
    ...
    // モックの設定
    userRepository.findByDiscordId.mockResolvedValue(new User(existingUserId, discordId));
    studentCardRepository.findByStudentId.mockResolvedValue(null);
    studentCardRepository.findByUserId.mockResolvedValue(null);
    studentCardRepository.create.mockResolvedValue(new StudentCard(2, studentId, existingUserId));
    ...
    expect(studentCardRepository.findByStudentId).toHaveBeenCalledWith(studentId);
    expect(studentCardRepository.findByUserId).toHaveBeenCalledWith(existingUserId);
    expect(studentCardRepository.create).toHaveBeenCalledWith(studentId, existingUserId);
  });
  ...
  it("既存ユーザーの学生証を更新できること", async () => {
    ...
    const oldStudentCard = new StudentCard(existingStudentCardId, oldStudentId, existingUserId);
    ...
    expect(result.isOk()).toBe(true);
    expect(userRepository.findByDiscordId).toHaveBeenCalledWith(discordId);
    expect(studentCardRepository.findByStudentId).toHaveBeenCalledWith(newStudentId);
    expect(studentCardRepository.findByUserId).toHaveBeenCalledWith(existingUserId);
  ...
  it("既に登録されている学生証番号に対してエラーを返すこと", async () => {
    ...
    expect(userRepository.findByDiscordId).toHaveBeenCalledWith(discordId);
    expect(studentCardRepository.findByStudentId).toHaveBeenCalledWith(duplicateStudentId);
  });
