...
export class RegisterStudentCardUseCase {
  ...
  async execute(
    ...
    try {
      ...
      const oldStudentCard = await this.studentCardRepository.findByUserId(user.id);
      ...
      const error = new RegisterStudentCardError("Failed to register student card.", {
        cause,
        meta: {
          code: "UNKNOWN",
        },
...
export class RegisterStudentCardError extends AppError {
  ...
  constructor(message: string, { meta, ...options }: RegisterStudentCardErrorOptions) {
