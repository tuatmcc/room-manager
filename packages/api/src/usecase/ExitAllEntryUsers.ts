...
export class ExitAllEntryUsersUseCase {
  ...
  async execute(): Promise<Result<ExitAllEntryUsersResult, ExitAllEntryUsersError>> {
    try {
      ...
      const users = await this.userRepository.findByIds(entryLogs.map((log) => log.userId));
      ...
      this.logger.error("failed to exit all entry users", serializeError(caughtError));
      const error = new ExitAllEntryUsersError("Failed to exit all entry users.", {
        cause,
        meta: {
          code: "UNKNOWN",
        },
...
export class ExitAllEntryUsersError extends AppError {
  ...
  constructor(message: string, { meta, ...options }: ExitAllEntryUsersErrorOptions) {
