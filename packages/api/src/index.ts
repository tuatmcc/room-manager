...
function createAppContext(env: Env, logger: AppLogger) {
  ...
  const repositories = createRepositories(db, logger.child({ tag: "repositories" }));
  const services = createServices(env, logger.child({ tag: "services" }));
  const usecases = createUseCases(repositories, logger.child({ tag: "usecases" }));
...
const app = new Hono<AppEnv>()
  ...
  .post("/local-device/touch-card", async (c) => {
    ...
    const localDeviceHandlers = createLocalDeviceHandlers(usecases, services, env, logger);
...
const scheduled: ExportedHandlerScheduledHandler<Env> = (_event, unknownEnv, ctx) => {
  const logger = createLogger({
    tag: "api:scheduled",
    context: {
