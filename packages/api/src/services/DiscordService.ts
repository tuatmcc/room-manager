...
export class DiscordService {
  ...
  async fetchUserInfo(userId: string): Promise<{ iconUrl: string; name: string }> {
  ...
  private getAvatarUrl(member: APIGuildMember): string {
    if (member.avatar != null) {
      return this.restClient.cdn.guildMemberAvatar(this.guildId, member.user.id, member.avatar, {
        size: 512,
      });
    }
    ...
    return this.restClient.cdn.defaultAvatar(calculateUserDefaultAvatarIndex(member.user.id));
  }
}
